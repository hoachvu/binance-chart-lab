...
export default function IndicatorControls({kind,settings,change}:Props){
  ...
  return <div className="indicator-settings" aria-label="Tùy chỉnh chỉ báo">
    <div className="setting-grid">
      ...
      {kind==="bb"&&<>{number("Chu kỳ",settings.bb.period,2,500,"period")}{number("Độ lệch chuẩn",settings.bb.deviation,.1,10,"deviation",.1)}{method(settings.bb.method)}{source(settings.bb.source)}{width(settings.bb.width)}{color("Màu dải",settings.bb.color,"color")}</>}
      {kind==="rsi"&&<>{number("Chu kỳ",settings.rsi.period,2,200,"period")}{source(settings.rsi.source)}{number("Quá mua",settings.rsi.overbought,settings.rsi.oversold+1,99,"overbought")}{number("Quá bán",settings.rsi.oversold,1,settings.rsi.overbought-1,"oversold")}{width(settings.rsi.width)}{color("Màu đường",settings.rsi.color,"color")}</>}
      {kind==="volume"&&<>{color("Nến tăng",settings.volume.upColor,"upColor")}{color("Nến giảm",settings.volume.downColor,"downColor")}</>}
    </div>
